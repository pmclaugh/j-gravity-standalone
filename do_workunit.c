...
void do_workunit(t_standalone *sim, t_workunit *w, int cq_id)
{
    int err;
    ...
    clEnqueueWriteBuffer(sim->context->cq[cq_id], d_N_start, CL_FALSE, 0, sizeof(cl_float4) * ncount, w->N, 0, NULL, &(loadevents[0]));
    clEnqueueWriteBuffer(sim->context->cq[cq_id], d_M, CL_FALSE, 0, sizeof(cl_float4) * mcount, w->M, 0, NULL, &(loadevents[1]));
    clEnqueueWriteBuffer(sim->context->cq[cq_id], d_V_start, CL_FALSE, 0, sizeof(cl_float4) * ncount, w->V, 0, NULL, &(loadevents[2]));
    ...
    err = clEnqueueNDRangeKernel(sim->context->cq[cq_id], sim->kernel, 1, NULL, &global, &local, 3, loadevents, &compute);
    clEnqueueReadBuffer(sim->context->cq[cq_id], d_N_end, CL_FALSE, 0, sizeof(cl_float4) * w->n_count, w->N, 1, &compute, &(w->done[0]));
    clEnqueueReadBuffer(sim->context->cq[cq_id], d_V_end, CL_FALSE, 0, sizeof(cl_float4) * w->n_count, w->V, 1, &compute, &(w->done[1]));
    clRetainEvent(w->done[0]);
    clRetainEvent(w->done[1]);
